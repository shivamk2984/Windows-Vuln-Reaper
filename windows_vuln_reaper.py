...
DB_LINK = "https://github.com/bitsadmin/wesng/raw/master/definitions.zip"
...
class CoreScanner:
    ...
    def load_defs(self):
        self._log("[*] Downloading latest definition database...")
        try:
            with urllib.request.urlopen(DB_LINK) as resp:
                if resp.status != 200:
                    self._log(f"[-] HTTP {resp.status}")
                    return None
                data = resp.read()
            
            self._log("[*] Extracting database in memory...")
            ...
            self._log(f"[-] Connection/Load Error: {ex}")
            return None
...
class AppWindow:
    def __init__(self, master):
        self.master = master
        self.master.title("Windows Vuln-Reaper")
        ...
        if os.path.exists(logo_path):
            try:
                ...
                # Resize specifically for window icon if needed, or keeping it as is.
                # User asked to make logo a bit big "in app". The window icon (top left) is usually small (16x16/32x32).
                # If "in app" means a visible logo, we might need to place it in the UI.
                # Assuming user means the icon used by the window manager:
    ...
    def build_ui(self):
        ...
        tk.Label(head_box, text="WINDOWS VULN-REAPER", font=("Consolas", 14, "bold"), bg=self.colors["bg"], fg=self.colors["accent"]).pack(anchor="w")
        ...
        ttk.Button(act_box, text="UPDATE DB", command=self.refresh_db, width=12).pack(side=tk.RIGHT, padx=5)
        ...
        self.switch_input()
        self.log_msg("Engine Initialized. Waiting for database download...")
    ...
    def generate_html_report(self):
        ...
        fname = "security_audit_report.html"
        try:
    ...
    def refresh_db(self):
        threading.Thread(target=self.core.fetch_definitions, daemon=True).start()
    ...
    def scan_thread(self, fpath):
        ...
        if hasattr(self, 'core_db') and self.core_db:
            ...
            if not db:
                 self.log_msg("[-] Critical: Failed to load vulnerability definitions.")
                 self.master.after(0, lambda: self.btn_go.state(['!disabled']))
                 return
            self.core_db = db
